...
namespace Ishopping.Controllers
{
    ...
    public class SkillOptionController : Controller
    {       
        ...
        private const int viewCod = 34; 

        public SkillOptionController(           
        ...
        public async Task<ActionResult> Alter()
        {
            ...
            var optionViewModel = new SkillOptionViewModel();
            optionViewModel.BasicUserViewItem = await _configUserViewItem.GetBasicViewItemAsync(viewCod, userId);
            optionViewModel.ComponentSkillOption = await _componentSkillOption.GetDefaultAsync(userId);
        ...
        public async Task<JsonResult> Salvar(string textView, string styleTextView, string subTitleView, string styleSubTitleView,
                string category, string description, string level)
        {
            string userId = User.Identity.GetUserId();
            var profile = await _userRegisterProfile.GetBasicProfileAsync(userId);
            ...
            {                              
                ...
                JsonResponse json = await _componentSkillOption.AppUpdateAsync(category, description, level, userId);
                return Json(json, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
                LogError.WhiteError(GetPathToLogError(), ex.ToString(), "SkillOptionController", "Salvar", profile.SiteNumber.ToString());
