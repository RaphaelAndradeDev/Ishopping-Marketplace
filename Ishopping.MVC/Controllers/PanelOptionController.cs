...
namespace Ishopping.Controllers
{
    ...
    public class PanelOptionController : Controller
    {     
        ...
        private const int viewCod = 28; 

        public PanelOptionController(     
        ...
        public async Task<ActionResult> Alter()
        {
            ...
            var optionViewModel = new PanelOptionViewModel();
            optionViewModel.BasicUserViewItem = await _configUserViewItem.GetBasicViewItemAsync(viewCod, userId);
            optionViewModel.ComponentPanelOption = await _componentPanelOption.GetDefaultAsync(userId);
        ...
        public async Task<JsonResult> Salvar(string textView, string styleTextView, string subTitleView, string styleSubTitleView, string title, string text)
        {
            string userId = User.Identity.GetUserId();
            var profile = await _userRegisterProfile.GetBasicProfileAsync(userId);
            ...
            {                            
                ...
                JsonResponse json = await _componentPanelOption.AppUpdateAsync(title, text, userId);
                return Json(json, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
                LogError.WhiteError(GetPathToLogError(), ex.ToString(), "PanelOptionController", "Salvar", profile.SiteNumber.ToString());
