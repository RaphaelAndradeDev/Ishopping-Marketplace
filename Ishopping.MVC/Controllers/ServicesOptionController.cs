...
namespace Ishopping.Controllers
{
    ...
    public class ServicesOptionController : Controller
    {       
        ...
        private const int viewCod = 33;  

        public ServicesOptionController(           
        ...
        public async Task<ActionResult> Alter()
        {
            ...
            var optionViewModel = new ServiceOptionViewModel();
            optionViewModel.BasicUserViewItem = await _configUserViewItem.GetBasicViewItemAsync(viewCod, userId);
            optionViewModel.ComponentServiceOption = await _componentServiceOption.GetDefaultAsync(userId);
        ...
        public async Task<JsonResult> Salvar(string textView, string styleTextView, string subTitleView, string styleSubTitleView, string title, string description)
        {
            ...
            {                          
                ...
                JsonResponse json = await _componentServiceOption.AppUpdateAsync(title, description, userId);
                return Json(json, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
                LogError.WhiteError(GetPathToLogError(), ex.ToString(), "ServiceOptionController", "Salvar", profile.SiteNumber.ToString());
